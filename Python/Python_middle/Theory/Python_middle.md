...
- Отсутствует параметр self в методах set_time и tell_time
- Для инициализации объекта требуется два аргумента
- Метод __init__ должен возвращать self
- Атрибут _time нельзя инициализировать строкой
- В метод set_time можно передать только строку
...
- Использование астериска импортирует только объекты из списка __all__
...
- [i**2 for i in range(5,26) if (i**0.5).is_integer()]
- [i for i in range(5,26) if (i**0.5).is_integer()]
- [i for i in range(5,25) if round(i**0.5)**2 == i]
- [i*2 for i in range(5,26) if (i**0.5).is_integer()]
...
small_circle_radius = 0.001
small_circle_area = calc_circle_area(small_circle_radius)
```
